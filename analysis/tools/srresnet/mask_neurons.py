...
def mask_neurons_with_different_proportion(baseline_model_path,
    ...
    for proportion in [1, 3, 5, 10]:
        ...
        for i in range(len(noisy_layer_neuron)):
            ...
            new_net_state_dict[conv_key][row, column, :, :] = baseline_state_dict[conv_key][row, column, :, :]
        ...
        for i in range(len(blurry_layer_neuron)):
            # locate weight params
            layer = blurry_layer_neuron[i]
            conv_key = conv_name_list[layer]
            row = blurry_row_neuron[i]
            column = blurry_column_neuron[i]
            new_net_state_dict[conv_key][row, column, :, :] = baseline_state_dict[conv_key][row, column, :, :]
