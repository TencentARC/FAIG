...
def calc_blur_gradient_loss(net_type, target_blur_folder, faig_folder, save_faig_maskdeblurfilter_blur_loss_txt,
    ...
    for proportion in [1, 3, 5, 10]:
        ...
        for idx, sub_folder in enumerate(sub_func_folder_names):
            ...
            faig_gradient_loss /= len(deblur_func_imglist)
            ig_gradient_loss /= len(deblur_func_imglist)
            abs_filter_change_gradient_loss /= len(deblur_func_imglist)
            random_gradient_loss /= len(deblur_func_imglist)

            if idx == 0:
                print('Calculate the effectiveness of masking discovered deblur filters for deblur function. '
                ...
                faig_maskdeblurfilter_blur_loss.append(faig_gradient_loss)
                ig_maskdeblurfilter_blur_loss.append(ig_gradient_loss)
                abs_filter_change_maskdeblurfilter_blur_loss.append(abs_filter_change_gradient_loss)
                random_maskdeblurfilter_blur_loss.append(random_gradient_loss)
            else:
                print('Calculate the effectiveness of masking discovered denoise filters for deblur function. '
                      'Lower value is better!')
                print(f'faig:{round(faig_gradient_loss, 4)}\t ig:{round(ig_gradient_loss, 4)}\t '
                      f'abs_filter_change:{round(abs_filter_change_gradient_loss, 4)}\t '
                      f'random:{round(random_gradient_loss, 4)}')

                faig_maskdenoisefilter_blur_loss.append(faig_gradient_loss)
                ig_maskdenoisefilter_blur_loss.append(ig_gradient_loss)
                abs_filter_change_maskdenoisefilter_blur_loss.append(abs_filter_change_gradient_loss)
                random_maskdenoisefilter_blur_loss.append(random_gradient_loss)

    faig_maskdeblurfilter_blur_loss = np.array(faig_maskdeblurfilter_blur_loss)
    faig_maskdenoisefilter_blur_loss = np.array(faig_maskdenoisefilter_blur_loss)

    ig_maskdeblurfilter_blur_loss = np.array(ig_maskdeblurfilter_blur_loss)
    ig_maskdenoisefilter_blur_loss = np.array(ig_maskdenoisefilter_blur_loss)

    abs_filter_change_maskdeblurfilter_blur_loss = np.array(abs_filter_change_maskdeblurfilter_blur_loss)
    abs_filter_change_maskdenoisefilter_blur_loss = np.array(abs_filter_change_maskdenoisefilter_blur_loss)

    random_maskdeblurfilter_blur_loss = np.array(random_maskdeblurfilter_blur_loss)
    random_maskdenoisefilter_blur_loss = np.array(random_maskdenoisefilter_blur_loss)

    # write the result to txt
    np.savetxt(save_faig_maskdeblurfilter_blur_loss_txt, faig_maskdeblurfilter_blur_loss, delimiter=',', fmt='%.6f')
    np.savetxt(save_faig_maskdenoisefilter_blur_loss_txt, faig_maskdenoisefilter_blur_loss, delimiter=',', fmt='%.6f')

    np.savetxt(save_ig_maskdeblurfilter_blur_loss_txt, ig_maskdeblurfilter_blur_loss, delimiter=',', fmt='%.6f')
    np.savetxt(save_ig_maskdenoisefilter_blur_loss_txt, ig_maskdenoisefilter_blur_loss, delimiter=',', fmt='%.6f')

    np.savetxt(
        save_abs_filter_change_maskdeblurfilter_blur_loss_txt,
        abs_filter_change_maskdeblurfilter_blur_loss,
        delimiter=',',
        fmt='%.6f')
    np.savetxt(
        save_abs_filter_change_maskdenoisefilter_blur_loss_txt,
        abs_filter_change_maskdenoisefilter_blur_loss,
        delimiter=',',
        fmt='%.6f')

    np.savetxt(save_random_maskdeblurfilter_blur_loss_txt, random_maskdeblurfilter_blur_loss, delimiter=',', fmt='%.6f')
    np.savetxt(
        save_random_maskdenoisefilter_blur_loss_txt, random_maskdenoisefilter_blur_loss, delimiter=',', fmt='%.6f')
